...
describe("Class transaction with debit.balance", function(){
  this.timeout(8000);
  ...
  xit("Transaction create with activated credit", async function() {
    await defaultCustomer.allowCredit(true);
    const tx = await transaction.Transaction.authorize(defaultCustomer,default_card_invoice,10,paymentOpts)
    defaultCustomer = await customer.Customer.get(tx.customer);
  ...
  xit("Transaction create with exceeded credit limit throw an error", async function() {
    try{
      const tx = await transaction.Transaction.authorize(defaultCustomer,default_card_invoice,30.1,paymentOpts)
      should.not.exist(tx);
    }catch(err) {
