...
Wallet.statics.create = function(options,callback){
  ...
  walletOptions.wid=VALID_PREFIX.account+tools.stringHash(config.option('apikey')+Date.now(),'base64')
...
Wallet.statics.retrieveAllGift = function (callback) {
  var promise = new mongoose.Promise, query={};
  if(callback){promise.addBack(callback);}
  ...
  this.find({apikey:apikey,giftcode:true}).lean().exec(function (err,wallets) {
    if(err){return promise.reject(err);}
