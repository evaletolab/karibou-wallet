...
Wallet.statics.create = function(options,callback){
  ...
  walletOptions.wid=VALID_PREFIX.account+tools.stringHash(config.option('apikey')+Date.now()+options.id+tools.randomString(6),'base64')
...
Wallet.statics.transfer_create=function (wid, transfer,callback) {
  ...
  this.__retrieve({wid:wid}).then(function (wallet) {
    ...
    //
    // FIXME analyse this situation ! Should be checked and implicit in the transacion
    if(transfer.wallet){
      if(transfer.wallet===wid){
        return promise.reject(new Error("You have to specify a recipient different than your wallet"))        
      }
      self.retrieve(transfer.wallet).then(function(source) {
        when.resolve(null,wallet)
      },function (err) {
        when.reject(new Error("Specified recipient doesn't exist"))   // body...
      });
      return when;
    }
...
Wallet.statics.transfer_cancel=function (wid, transfer,callback) {
  ...
  var promise = new Promise, now=new Date(),self=this;
  if(callback){promise.addBack(callback);}
  ...
  this.__retrieve({wid:wid},function (err,wallet) {
    ...
    //
    // log
    transfered.logs.unshift("Reversed "+transfered.type+" "+round2digit(transfered.amount/100)+" CHF at "+now);
