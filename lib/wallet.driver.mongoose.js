...
Wallet.pre('save',function (next) {
    var stringHash=JSON.stringify(sortObject(this.toObject()));
    this.signed_wallet=tools.stringHash(config.option('secret')+stringHash,'base64');
...
Wallet.methods.verifySign=function function_name () {
  var stringToHash=JSON.stringify(sortObject(this.toObject()));
  var stringHash=tools.stringHash(config.option('secret')+stringToHash,'base64');
...
Wallet.statics.create = function(options,callback){
  ...
  var Wallet=this, 
      oneYear=new Date(Date.now()+86400000*365) ;
  ...
  if(options.external_account){
    ...
    ['swift','name','address1','address2'].forEach(function (field) {
  ...
  walletOptions.wid=VALID_PREFIX.account+tools.stringHash(config.option('secret')+Date.now()+'','base64')
  walletOptions.card.number=fnv.hash(walletOptions.wid,52).dec()+'';
  walletOptions.card.last4=walletOptions.card.number.substr(16 - 4);

  var sum=check.mod10check(walletOptions.card.number.substr(15),true);
  walletOptions.card.number += (10 - (sum % 10)) % 10;
  check.mod10check(walletOptions.card.number);

  //
  // attach the wallet
  if(options.id){
    walletOptions.id=options.id;
    walletOptions.card.registeredTo=walletOptions.id;
  }
...
Wallet.statics.transferWallet = function (wid,card,callback) {
  assert(wid);
  assert(card);
  assert(card.uid);
  assert(card.name);
  ...
  .then(function (giftcode) {
    if(giftcode.id || giftcode.card.registeredTo){
      return promise.reject(new Error("The card can not be transfered"));
    }
    giftcode.card.registeredTo=card.uid;
...
Wallet.statics.__retrieve=function (query,cb) {
  ...
  this.findOne(query)
      .select('+available +signed_wallet +transactions.id +transfers.id')
...
Wallet.statics.transaction_charge=function(wid,transaction,callback){
  ...
  var money=new Money(transaction.amount, Money.CHF),
      ...
      transactionOptions={
        id:VALID_PREFIX.transaction+tools.stringHash(config.option('secret')+Date.now()+'','base64'),
        amount:money.amount,
        amount_refunded:0,
  ...
  this.__retrieve(query,function (err,wallet) {
    ...
    //
    // log
    transactionOptions.logs.push(transactionOptions.status+" "+round2digit(transactionOptions.amount/100)+" CHF at "+transactionOptions.created);

    wallet.transactions.push(transactionOptions);

    wallet.save(function (err,wallet) {
      if(err){return promise.reject(err);}
      return promise.resolve(null,transactionOptions);
    });
  });
...
Wallet.statics.transaction_capture=function (wid, transaction,callback) {
  ...
  this.__retrieve({wid:wid},function (err,wallet) {
    ...
    //
    // log
    trans.logs.push(trans.status+" "+round2digit(trans.amount/100)+" CHF at "+now);
    wallet.save(function (err,wallet) {
      if(err){return promise.reject(err);}
...
Wallet.statics.transaction_refund=function (wid, transaction,callback) {
  ...
  this.__retrieve({wid:wid},function (err,wallet) {
    ...
    //
    // log
    trans.logs.push(trans.status+" "+round2digit(trans.amount_refunded/100)+" CHF at "+now);
...
Wallet.statics.transfer_create=function (wid, transfer,callback) {
  ...
  var money=new Money(transfer.amount, Money.CHF),
    ...
    transferOption={
      id:VALID_PREFIX.transfer+tools.stringHash(config.option('secret')+Date.now()+'','base64'),
      amount:money.amount,
      amount_reversed:0,
      reversed:false,
      application_fee:null,
  ...
  this.__retrieve({wid:wid},function (err,wallet) {
    ...
    //
    // log
    transferOption.logs.push(transferOption.type+" "+round2digit(transferOption.amount/100)+" CHF at "+transferOption.created);
...
Wallet.statics.transfer_cancel=function (wid, transfer,callback) {
  ...
  this.__retrieve({wid:wid},function (err,wallet) {
    ...
    wallet.save(function (err,wallet) {
      ...
      return promise.resolve(null,transfered);
    })
  });
