...
/**
 * ## check.getHiddenNumber(card)
 * *Return the hidden value of this card number*
 *
 * Example:
 *     check.getHiddenNumber('4111111111111111');
 *     // returns: '41XXXXXXXXXX1111'
 */
check.getHiddenNumber = function(card) {
  var nb = check.extractDigits(card),
      digits = nb.split(''); 
  for(var i=0;i<nb.length;i++){
    if(i>1 && i<(nb.length-4)){
      digits[i]='x'
    }
  }
  return digits.join('');
 ...
 */
check.mod10check = function(card,returnSum) {
    var sum = 0;
    var totalDigits;
    var oddEven;
    var digits, digit;
    ...
    if(returnSum)return sum;

    return (sum % 10) === 0 && card;
};

check.mod10gen =function (card, padding) {
  var digits = card.split(''); // Convert to array
  var pad="0000000000";
  var len=card.length;
  var len_offset = (len + 1) % 2, sum=0;

  for (var pos = 0; pos < len - 1; pos++) {
    if ((pos + len_offset) % 2) {
      t = parseInt(digits[pos],10) * 2;
      if (t > 9) {
        t -= 9;
      }
      sum += t;
      ...
      sum += parseInt(digits[pos],10);
    }
  }
  var final_digit = (10 - (sum % 10)) % 10;
  digits[len - 1] = final_digit;  
  padding=padding||16;
  return pad.substring(0, padding - digits.length)+digits.join('')
}
/**
...
/**
 * ## check.shaSign(paymentData)
 * *to sign postfinance payment with SHA256
 * 1) sort paymendData keys (uppperCase+alpha)
 * 2) stringify with this format KEY=VALUE+(SECRET||'')
 * 3) return sha
 */

check.shaSign=function(payload) {
  var postfinanceStr="", 
      secret=config.option('shaSecret'), 
      append=secret;

  //
  // configure sha without padding secret
  ...
  Object.keys(payload).map(function(k) {
   payloadFormatted[k.toUpperCase()] =payload[k];
  });

  // sort 
  Object.keys(payloadFormatted).sort().forEach(function(key){
    // avoid callback params from paramplus 
    if(['SHASIGN','CREATEALIAS','USER'].indexOf(key)!==-1){
      return;
    }
    if(payloadFormatted[key].length===0){
      return;
    }
    postfinanceStr+=key+'='+payloadFormatted[key]+append
  })

  // sign
  var SHASIGN=require('crypto').
    createHash('sha256').update(postfinanceStr,'utf8').digest("hex").toUpperCase()    

  debug('SHASTR: '+postfinanceStr)
  debug('SHASIGN: '+SHASIGN)
...
check.isValidSha=function(payload) {
  if(!payload||!payload.SHASIGN)
    return false;

  var sha=this.shaSign(payload)
  return payload.SHASIGN===sha;
}
